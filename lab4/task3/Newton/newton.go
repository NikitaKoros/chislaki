...
func main() {
	myApp := app.New()
	myWindow := myApp.NewWindow("Метод стрельбы - Метод Ньютона")
	...
	fmt.Println()
	fmt.Println(strings.Repeat("=", 140))
	fmt.Println("МЕТОД СТРЕЛЬБЫ ДЛЯ КРАЕВОЙ ЗАДАЧИ - МЕТОД НЬЮТОНА")
	fmt.Println(strings.Repeat("=", 140))
	fmt.Println()
	...
	// Информация
	infoText := `МЕТОД СТРЕЛЬБЫ ДЛЯ КРАЕВЫХ ЗАДАЧ ПЕРВОГО РОДА - МЕТОД НЬЮТОНА
...
3. МЕТОД НЬЮТОНА (итерационная процедура):
   - Более быстрая сходимость по сравнению с методом дихотомии
