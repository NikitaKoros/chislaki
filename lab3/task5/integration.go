...
func createMidpointPlot(fn func(float64) float64, a, b, h float64, width, height float64) *fyne.Container {
	...
	n := int(math.Ceil((b - a) / h))
	h = (b - a) / float64(n)
	rectColor := color.RGBA{100, 150, 255, 100} // Полупрозрачный синий
	...
	// График функции
	funcColor := color.RGBA{255, 0, 0, 255} // Красный
...
func createTrapezoidPlot(fn func(float64) float64, a, b, h float64, width, height float64) *fyne.Container {
	...
	n := int(math.Ceil((b - a) / h))
	h = (b - a) / float64(n)
	trapColor := color.RGBA{100, 255, 150, 100} // Полупрозрачный зеленый
	...
	for i := 0; i < n; i++ {
		...
		// Рисуем трапецию как полигон
		// Заполнение трапеции (приближение через много маленьких линий)
		steps := 50
		for j := 0; j < steps; j++ {
			...
			x1 := xLeft + t1*h
			x2 := xLeft + t2*h
			// Линейная интерполяция высоты трапеции
...
func createSimpsonPlot(fn func(float64) float64, a, b, h float64, width, height float64) *fyne.Container {
	...
	// Параболы (метод Симпсона использует параболы через 3 точки)
	n := int(math.Ceil((b - a) / h))
	if n%2 != 0 {
		n++
	}
	...
	parabolaColor := color.RGBA{255, 150, 0, 255} // Оранжевый

	// Рисуем параболы через каждые 2 интервала
	for i := 0; i < n; i += 2 {
		x0 := a + float64(i)*h
		x1 := a + float64(i+1)*h
		x2 := a + float64(i+2)*h
		...
		// Построение параболы через 3 точки: y = ax² + bx + c
		// Используем интерполяцию Лагранжа
...
func createEulerPlot(fn func(float64) float64, a, b, h float64, width, height float64) *fyne.Container {
	...
	n := int(math.Ceil((b - a) / h))
	h = (b - a) / float64(n)
	tangentColor := color.RGBA{128, 0, 255, 255} // Фиолетовый
	...
	for i := 0; i <= n; i++ {
		...
		// Рисуем касательную линию в окрестности точки
		tangentLen := h * 0.5
...
func main() {
	...
	a, b := -2.0, 2.0
	h1 := 0.4 // Шаг выбран так, чтобы можно было применить все методы
	...
	comparisonContainer := container.NewVScroll(widget.NewLabel(comparisonText))

	infoText := createInfoText()
	infoContainer := container.NewVScroll(widget.NewLabel(infoText))

	// Создание вкладок с детальными результатами и графиками для каждого метода
	...
	midpointPlot := createMidpointPlot(integrandF, a, b, h1, 1100, 600)
	midpointContent := container.NewVBox(
		widget.NewLabel("МЕТОД СРЕДНИХ ПРЯМОУГОЛЬНИКОВ"),
		widget.NewSeparator(),
		midpointPlot,
		widget.NewSeparator(),
		widget.NewLabel(midpointDetails),
	)
	midpointContainer := container.NewVScroll(midpointContent)
	...
	trapezoidPlot := createTrapezoidPlot(integrandF, a, b, h1, 1100, 600)
	trapezoidContent := container.NewVBox(
		widget.NewLabel("МЕТОД ТРАПЕЦИЙ"),
		widget.NewSeparator(),
		trapezoidPlot,
		widget.NewSeparator(),
		widget.NewLabel(trapezoidDetails),
	)
	trapezoidContainer := container.NewVScroll(trapezoidContent)
	...
	simpsonPlot := createSimpsonPlot(integrandF, a, b, h1, 1100, 600)
	simpsonContent := container.NewVBox(
		widget.NewLabel("МЕТОД СИМПСОНА"),
		widget.NewSeparator(),
		simpsonPlot,
		widget.NewSeparator(),
		widget.NewLabel(simpsonDetails),
	)
	simpsonContainer := container.NewVScroll(simpsonContent)

	eulerDetails := createMethodDetailsTable("Метод Эйлера", resultsH1.euler, resultsH2.euler, resultsRR.eulerRR, exact, 2, h1)
	eulerPlot := createEulerPlot(integrandF, a, b, h1, 1100, 600)
	eulerContent := container.NewVBox(
		widget.NewLabel("МЕТОД ЭЙЛЕРА (УТОЧНЕННЫЙ МЕТОД ТРАПЕЦИЙ)"),
		widget.NewSeparator(),
		eulerPlot,
		widget.NewSeparator(),
		widget.NewLabel(eulerDetails),
	)
	eulerContainer := container.NewVScroll(eulerContent)

	tabs := container.NewAppTabs(
		container.NewTabItem("Результаты", resultsContainer),
		container.NewTabItem("Сравнение", comparisonContainer),
		container.NewTabItem("Прямоугольники", midpointContainer),
		container.NewTabItem("Трапеции", trapezoidContainer),
		container.NewTabItem("Симпсон", simpsonContainer),
		container.NewTabItem("Эйлер", eulerContainer),
